        ...
        <div className="flex flex-col md:flex-row gap-8 items-start md:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-3 text-xs font-medium text-slate-600 dark:text-slate-300">
                ...
                <div className="flex items-center gap-2 bg-gray-100 dark:bg-slate-900 p-1.5 rounded-lg">
                    {/* UPDATED: Increased button sizes to 40px (w-10 h-10) to meet touch target guidelines */}
                    <button onClick={() => setBgColor('transparent')} aria-label="Transparent background" className={`w-10 h-10 rounded-md flex items-center justify-center border ${bgColor === 'transparent' ? 'bg-white shadow-sm border-gray-200' : 'border-transparent opacity-50'}`}><div className="w-6 h-6 bg-gray-300 rounded-sm" style={{backgroundImage: 'linear-gradient(45deg, #ccc 25%, transparent 25%), linear-gradient(-45deg, #ccc 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #ccc 75%), linear-gradient(-45deg, transparent 75%, #ccc 75%)', backgroundSize: '4px 4px'}}></div></button>
                    <button onClick={() => setBgColor('#ffffff')} aria-label="White background" className={`w-10 h-10 rounded-md border bg-white ${bgColor === '#ffffff' ? 'shadow-sm border-gray-300 ring-1 ring-slate-200' : 'border-transparent opacity-50'}`}></button>
                    <button onClick={() => setBgColor('#000000')} aria-label="Black background" className={`w-10 h-10 rounded-md border bg-black ${bgColor === '#000000' ? 'shadow-sm border-gray-600 ring-1 ring-slate-700' : 'border-transparent opacity-50'}`}></button>
                </div>
            </div>
        </div>
      </div>
